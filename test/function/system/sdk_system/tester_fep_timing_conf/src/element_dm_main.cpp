...
{
    fep::cModuleOptions oModuleOptions("DummyElement");
    ...
    while (XARG_AVAILABLE())
    {
        ...
        else if (strCurrentArg == "-steps")
        {
            ...
            if (nNumberOfSteps != 1 && 
                ...
                nNumberOfSteps != 64)
            {
                std::cerr << "Invalid number of steps. Valid numbers are 1,2,4,8,16,32,64" << std::endl;
                return 1;
            }
        }
    ...
    {
        // Decide on config
        strTimingConfiguration = a_util::strings::format("files/dmmy_configuration_%d.xml", nNumberOfSteps);
    }

    std::string strElementName = "DummyElement";
    if (nElementNumber > 0)
    {
        strElementName = a_util::strings::format("DummyElement%d", nElementNumber);
    }
