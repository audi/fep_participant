...
{
    fep::cModuleOptions oModuleOptions("ClientElement");
    ...
    while (XARG_AVAILABLE())
    {
        ...
        else if (strCurrentArg == "-steps")
        {
            ...
            if (nNumberOfSteps != 1 && 
                ...
                nNumberOfSteps != 8)
            {
                std::cerr << "Invalid number of steps. Valid numbers are 1,2,4,8" << std::endl;
                return 1;
            }
        }
    ...
    {
        // Decide on config
        strTimingConfiguration = a_util::strings::format("files/mult_configuration_%d.xml", nNumberOfSteps);
    }

    std::string strElementName = "DummyElement";
    if (nElementNumber > 0)
    {
        strElementName = a_util::strings::format("ClientElement%d", nElementNumber);
    }
    ...
    if (fep::isOk(result))
    {
        ...
        result |= oElement.GetPropertyTree()->SetPropertyValue(DUMMY_ELEMENT_NUMBER_OF_STEPS_PROPERTY, nNumberOfSteps);
    }
