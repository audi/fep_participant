 ...
 * @req_id ""
 */
TEST(fast_spinlock_test, TestSpinlock)
{
    SpinlockTestStruct t;
    ASSERT_TRUE(t.mtx.try_lock());

    {
        concurrency::thread th(&SpinlockTestStruct::Work, &t);
...
TEST(fast_spinlock_test, TestRecursiveSpinlock)
{
#ifdef WIN32
    // Only the windows spinlock is recursive
    spinlock m;
    m.lock();
    m.lock();
    m.unlock();
