...
namespace internal
{
//resuse internal GTest stuff
extern const char* ParseFlagValue(const char*, const char*, bool);
extern bool ParseBoolFlag(const char*, const char*, bool*);
extern bool ParseInt32Flag(const char*, const char*, Int32*);
} // namespace internal
} // namespace testing
...
namespace
{
/// Severity levels in case of a missing/incorrect command line parameter
enum SeverityLevel
{
    IGNORE_IF_MISSING = 0x00, //!< Ignore, do nothing else
    LOG_IF_MISSING = 0x01,    //!< Print a log message and continue
    EXCEPT_IF_MISSING = 0x02, //!< Print a log message and execute gtest's @c EXPECT_TRUE(false)
    ASSERT_IF_MISSING = 0x03  //!< Print a log message and execute gtest's @c ASSERT_TRUE(false)
};

inline bool ParseStringFlag(const char* str, const char* flag, const char** value)
{
    const char* const value_str = ::testing::internal::ParseFlagValue(str, flag, false);
    if (value_str == NULL)
    {
        return false;
    }
...
class GTestCommandLine
{
public:
    const char* parseStringFlag(const char* flag,
        SeverityLevel severity_level = IGNORE_IF_MISSING) const
    {
        // using ::testing::internal::ParseStringFlag;
        // return parseFlag<std::string>(flag, severity_level, &ParseStringFlag);
        return parseFlag<const char*>(flag, severity_level, &ParseStringFlag);
    }

    bool parseBoolFlag(const char* flag, SeverityLevel severity_level = IGNORE_IF_MISSING) const
    {
        using ::testing::internal::ParseBoolFlag;
        return parseFlag<bool>(flag, severity_level, &ParseBoolFlag);
    }

    int parseInt32Flag(const char* flag, SeverityLevel severity_level = IGNORE_IF_MISSING) const
    {
        using ::testing::internal::ParseInt32Flag;
        return parseFlag< ::testing::internal::Int32>(flag, severity_level, &ParseInt32Flag);
    }

protected:
    // overload for error handling
    virtual void onRequestedFlagMissing(const char* flag, SeverityLevel severity_level) const
    {
        if (IGNORE_IF_MISSING == severity_level)
        {
            return;
        }

        std::string message = std::string("Command line flag not found: --") + flag;
        if (LOG_IF_MISSING == severity_level)
        {
            message += " (ignore)";
            GTEST_PRINTF(message.c_str());
        }
        else if (EXCEPT_IF_MISSING == severity_level)
        {
            message += " (non_fatal)";
            GTEST_PRINTF(message.c_str());
            EXPECT_TRUE(false);
...
private:
    const char* makeFlagGTestConform(const std::string& str) const
    {
        // internal only works with GTEST_FLAG_PREFIX_ prefix, so insert it
        // arguments must start with "--", otherwise it is ill-formed
        _current_gtest_conform_command_line_parameter =
            std::string("--") + GTEST_FLAG_PREFIX_ + str.substr(2);
        return _current_gtest_conform_command_line_parameter.c_str();
    }

    template <typename ValueType>
    ValueType parseFlag(const char* flag,
        SeverityLevel severity_level,
        bool(*parse_function)(const char*, const char*, ValueType*)) const
    {
        using ::testing::internal::GetArgvs;
        typedef std::vector< ::testing::internal::string>::const_iterator argIterator;

        for (argIterator it = GetArgvs().begin(); GetArgvs().end() != it; ++it)
        {
            ValueType value;
            if ((*parse_function)(makeFlagGTestConform(*it), flag, &value))
            {
                return value;
            }
        }

        onRequestedFlagMissing(flag, severity_level);
        return ValueType();
    }

private:
    mutable std::string _current_gtest_conform_command_line_parameter;
};
...
/**
 * This macro checks if the return value of the expression is false (bool), in which case
 * it will throw an exception. The exception will be caught by the test bench and hence
 * allows to verify statements in sub-functions which are only indirectly part of the test case.
 *
...
#endif //ASSERT_OR_THROW
/**
 * The same as @ref ASSERT_OR_THROW with an additional message.
 *
 * @param [in] expression      The expression which will be excecuted
 * @param [in] msg             Additional information which will be logged
 */
#ifndef ASSERT_OR_THROW_EXT
#define ASSERT_OR_THROW_EXT(expression, msg)                                                       \
    if (!(expression))                                                                             \
    {                                                                                              \
        GTEST_PRINTF(msg);                                                                         \
        ADD_FAILURE_AT(__FILE__, __LINE__);                                                        \
...
#endif //ASSERT_OR_THROW_EXT

/**
 * This macro checks if the return value of the expression equals ERR_NOERROR, in which case
 * it will throw an exception. The exception will be caught by the test bench and hence
 * allows to verify statements in sub-functions which are only indirectly part of the test case.
 *
 * @param [in] expression      The expression which will be excecuted
 */
#ifndef ASSERT_RESULT_OR_THROW
#define ASSERT_RESULT_OR_THROW(expression) ASSERT_OR_THROW((expression) == ERR_NOERROR)
#endif //ASSERT_RESULT_OR_THROW
...
public:
    ...
    virtual fep::Result Create(const cModuleOptions& oModuleOptions, ITransmissionDriver* pDriver=NULL, const bool platformDependentName = true) 
    {
        ...
        if (platformDependentName)
        {
        oMyModuleOptions.SetParticipantName(MakePlatformDepName(strModuleName.c_str()).c_str());
        }
...
public:
    fep::Result FillModuleHeader()
    {
        ...
        GetPropertyTree()->SetPropertyValue(fep::g_strElementHeaderPath_fFEPVersion, 100.0);
...
protected:
    ...
    virtual fep::Result ProcessInitializingEntry(const fep::tState eOldState)
    {
        ...
        if (m_bGoToRunning || m_bJumpOverInit)
        {
            if (m_bErrorHappenedInInit)
            {
...
public:
                                ...
                                const  char* strDescription = NULL)
    {
        ...
        m_sLastEvent.strFile = std::string(strFile ? strFile : "");
